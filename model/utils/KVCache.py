...
class ByteKVCache:
    """Transformer注意力层的键值(KV)缓存实现。

    该缓存为Transformer模型的自回归解码提供高效的内存管理，支持动态序列增长、
    束搜索重排序、状态保存/加载等关键操作。

    功能特性：
    - 预分配固定形状的缓冲区，避免解码过程中的重复内存分配
    - 支持单步追加和块追加操作
    - 提供束搜索重排序、缓存剪枝和设备管理功能
    - 支持状态序列化和分布式分片操作

    参数：
        num_layers: 需要缓存的Transformer层数
        num_heads: 每层的注意力头数量
        head_dim: 每个注意力头的特征维度
        max_seq_len: 缓存支持的最大序列长度
        batch_size: 支持的最大批处理大小
        dtype: 张量数据类型（如torch.float16）
        device: 缓存存储设备（默认为CPU）
    ...
    def _init_buffers(self) -> None:
        """为所有层预分配键(key)和值(value)缓存空间。
        
        每层缓存包含：
        - 'k': 键张量 [batch_size, num_heads, max_seq_len, head_dim]
        - 'v': 值张量 [batch_size, num_heads, max_seq_len, head_dim]
        - 'seq_len': 当前有效序列长度（初始为0）
        """
        self._layers = []
    ...
    def append_batch(
        ...
        将新的键值对追加到当前序列末尾，并更新序列长度指针。
        
        参数：
            layer_idx: 目标层索引
            k: 待追加的键张量 [B, num_heads, L_block, head_dim]
            v: 待追加的值张量 [B, num_heads, L_block, head_dim]
            batch_index: 批处理索引 [B]，指定写入的批次位置（默认连续写入）
        
        返回：
            int: 更新后的序列长度
        
        异常：
            ValueError: 输入维度不匹配或层索引无效
            RuntimeError: 追加后超出缓存容量
        """
        # 验证层索引有效性
    ...
    def set_slice(
        ...
        参数：
            ...
            batch_index: 批处理索引 [B]（默认连续写入）
        
        异常：
            ValueError: 参数无效
            RuntimeError: 写入位置超出缓存边界
        """
        # 验证参数有效性
        if not 0 <= layer_idx < self.num_layers:
            raise ValueError(f"层索引{layer_idx}超出范围[0, {self.num_layers-1}]")
        if start < 0:
            raise ValueError(f"起始位置不能为负, 实际值: {start}")
        ...
        if end_pos > self.max_seq_len:
            raise RuntimeError(
                f"写入位置超出缓存边界: 结束位置={end_pos}, 最大容量={self.max_seq_len}"
            )
    ...
    def get_kv(
        ...
        参数：
            layer_idx: 目标层索引
            max_len: 返回的最大序列长度（默认返回当前全部有效长度）
        
        返回：
            Tuple[torch.Tensor, torch.Tensor]: (k, v) 元组，
            形状为 [batch, num_heads, seq_len, head_dim]
        """
        layer = self._layers[layer_idx]
        current_len = int(layer["seq_len"])
        if current_len == 0:
        ...
        if seq_len > current_len:
            raise ValueError(f"请求长度{seq_len}超过当前缓存长度{current_len}")
    ...
    def reorder(self, new_order: torch.Tensor) -> None:
        ...
        异常：
            TypeError: 索引类型错误
            ValueError: 批处理大小超过缓存容量
        """
        # 索引类型校验
        if new_order.dtype != torch.long:
            raise TypeError("重排索引必须是torch.long型")
        
        batch_size = new_order.shape[0]
        # 批处理大小校验
        if batch_size > self.batch_size:
            raise ValueError(f"重排批大小{batch_size}超过缓存容量{self.batch_size}")
    ...
    def prune(self, new_max_len: int) -> None:
        ...
        异常：
            ValueError: 新长度超出有效范围
        """
        if not 0 < new_max_len <= self.max_seq_len:
            raise ValueError(f"新长度{new_max_len}必须在(0, {self.max_seq_len}]范围内")
